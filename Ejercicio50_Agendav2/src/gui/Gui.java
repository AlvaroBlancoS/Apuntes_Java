...
public class Gui {
	...
	private static boolean elegirOpcion(int opcion) {
		...
		switch (opcion) {
		case 0:
			resp = Agregar.leerString("Estás seguro que quieres salir? si-no");
		...
		case 1:
			...
			if (d.isExist(nombre, apellidos) == false) {
				...
				throw new ExceptionEqualContact("No se ha podido registrar porque existe el contacto");
			}
		...
		default:
			Agregar.mensajeContacto().log(Level.ERROR, "Error!!!");
			break;
		}
		return sigo;
	}
